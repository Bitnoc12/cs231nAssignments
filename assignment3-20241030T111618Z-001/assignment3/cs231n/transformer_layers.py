...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        # cal posi
        col = torch.arange(max_len)[:, None] # 形状为 (max_len, 1)
        row = torch.pow(10000, -torch.arange(0, embed_dim, 2) / embed_dim) # 获取公式中的缩放系数
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        K = K.reshape(N, T, H, D).transpose(1, 2)
        V = V.reshape(N, T, H, D).transpose(1, 2)
        ...
        if attn_mask is not None:
          energy = energy.masked_fill(attn_mask == 0, -1e9) # 设置为一个非常小的数值（如 -1e9），可以有效避免数值溢出问题
