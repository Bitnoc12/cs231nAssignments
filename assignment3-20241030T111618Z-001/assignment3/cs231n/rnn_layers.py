...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    N, T, _ = x.shape # N 表示批大小，T 表示时间步数，D 表示输入特征的维度，H 表示隐藏状态的维度。
    _, H = h0.shape
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # Step 1: Compute the intermediate variable a
    a = x.dot(Wx) + prev_h.dot(Wh) + b
    # Step 2: Split the variable a into four parts, corresponding to the input gate, forget gate, output gate, and gate gate.
    ai, af, ao, ag = np.split(a, 4, axis=1)

    # Step 3: Apply activation functions to compute the gate activations
    i = sigmoid(ai)   # Input gate
    f = sigmoid(af)   # Forget gate
    o = sigmoid(ao)   # Output gate
    g = np.tanh(ag)   # Gate gate

    # Step 4: Compute the next cell state using the forget and input gates
    next_c = f * prev_c + i * g

    # Step 5: Compute the next hidden state using the output gate and the new cell state
    next_h = o * np.tanh(next_c)

    # Step 6: Store values needed for the backward pass in a cache
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # Step 1: Compute the gradient of next cell state, combining gradients from next hidden state and next cell state
    dnext_c += dnext_h * o * (1 - np.tanh(next_c) ** 2)

    # Step 2: Compute the gradient of previous cell state
    dprev_c = dnext_c * f

    # Step 3: Compute the gradients of the gates
    dai = dnext_c * g * i * (1 - i)
    daf = dnext_c * prev_c * f * (1 - f)
    dao = dnext_h * np.tanh(next_c) * o * (1 - o)
    dag = dnext_c * i * (1 - g ** 2)

    # Combine the gate gradients to get the total gradient of `a`
    da = np.concatenate((dai, daf, dao, dag), axis=1)

    # Step 4: Compute the gradient of the input data `x`
    dx = da.dot(Wx.T)

    # Step 5: Compute the gradient of the previous hidden state
    dprev_h = da.dot(Wh.T)

    # Step 6: Compute the gradient of the weight matrices
    dWx = x.T.dot(da)
    dWh = prev_h.T.dot(da)

    # Step 7: Compute the gradient of the bias
    db = np.sum(da, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    # Step 1: Initialize cell state `c` to zeros
    c = np.zeros((N, H))

    # Step 2: Initialize `h` to store all hidden states, shape `(N, T, H)`
    h = np.zeros((N, T, H))

    # Step 3: Initialize `cache` as an empty list
    cache = []

    # Step 4: Set `prev_h` to the initial hidden state `h0`
    prev_h = h0

    # Set `prev_c` to the initial cell state `c` (which is all zeros)
    prev_c = c

    # Step 5: Loop over each timestep `t`
    for t in range(T):
        # Compute the next hidden state and cell state using `lstm_step_forward`
        next_h, next_c, cache_t = lstm_step_forward(x[:, t, :], prev_h, prev_c, Wx, Wh, b)

        # Update `prev_h` and `prev_c` to be the current `next_h` and `next_c`
        prev_h = next_h
        prev_c = next_c

        # Store the current hidden state in `h` at timestep `t`
        h[:, t, :] = next_h

        # Store the current timestep's cache in `cache`
...
def lstm_backward(dh, cache):
    ...
    # Backward pass through the entire sequence
    for t in reversed(range(T)):
        # Accumulate gradient from the current timestep
        dnext_h += dh[:, t, :]
        
        # Call lstm_step_backward for the current timestep
        dx[:, t, :], dnext_h, dnext_c, dWx_t, dWh_t, db_t = lstm_step_backward(dnext_h, dnext_c, cache[t])
        
        # Accumulate weight and bias gradients over all timesteps
        dWx += dWx_t
        dWh += dWh_t
        db += db_t

    # The gradient with respect to the initial hidden state
    dh0 = dnext_h
